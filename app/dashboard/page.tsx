...
export default function EngiconnectDashboard() {
    ...
    useEffect(() => {
        ...
        const unsubSite = onSnapshot(collection(db, "appointments"), snap => {
            ...
            if (!hasGlobalAccess) { if (isTSM) docs = docs.filter((d: any) => d.submittedBy === userId || subordinateIds.includes(d.submittedBy)); else docs = docs.filter((d: any) => d.submittedBy === userId) }
        ...
        const unsubShop = onSnapshot(collection(db, "shop_drawing_requests"), snap => {
            let docs = snap.docs.map(d => ({ id: d.id, ...d.data() }))
            if (!hasGlobalAccess) docs = docs.filter((d: any) => d.submittedBy === userId || subordinateIds.includes(d.submittedBy))
        ...
        const unsubJob = onSnapshot(collection(db, "job_requests"), snap => {
            let docs = snap.docs.map(d => ({ id: d.id, ...d.data() }))
            if (!hasGlobalAccess) docs = docs.filter((d: any) => (d.submittedBy || d.createdBy) === userId || subordinateIds.includes(d.submittedBy || d.createdBy))
        ...
        return () => { unsubSite(); unsubShop(); unsubTesting(); unsubJob(); unsubOther(); unsubDialux(); supabase.removeChannel(ch) }
    ...
    useEffect(() => {
        ...
        const qSite = query(collection(db, "appointments"), orderBy("createdAt", "desc"), limit(10))
        const qTesting = query(collection(db, "testing_tracker"), orderBy("createdAt", "desc"), limit(10))
        const activitiesMap = new Map()
        function updateActivity(type: string, docs: any[]) {
            let filteredDocs = docs
            if (!hasGlobalAccess) {
                if (isTSM) filteredDocs = docs.filter(d => { const data = d.data(); const submittedBy = data.submittedBy || data.createdBy || data.UserId || data.userId || data.pic || data.assignedTo; return submittedBy === userId || subordinateIds.includes(submittedBy) })
                else filteredDocs = docs.filter(d => { const data = d.data(); const submittedBy = data.submittedBy || data.createdBy || data.UserId || data.userId || data.pic || data.assignedTo; return submittedBy === userId })
            }
            filteredDocs.forEach(doc => { const data = doc.data(); activitiesMap.set(doc.id, { id: doc.id, type, ...data }) })
        ...
        const unsubSite = onSnapshot(qSite, snap => updateActivity("Site Visit", snap.docs))
        const unsubTesting = onSnapshot(qTesting, snap => updateActivity("Testing", snap.docs))
    ...
    useEffect(() => {
        ...
        const unsubApps = onSnapshot(qApps, snap => {
            ...
            if (!hasGlobalAccess) { if (isTSM) apps = apps.filter((a: any) => a.submittedBy === userId || subordinateIds.includes(a.submittedBy)); else apps = apps.filter((a: any) => a.submittedBy === userId) }
        ...
        const unsubTesting = onSnapshot(qTesting, snap => {
            ...
            if (!hasGlobalAccess) { if (isTSM) items = items.filter((i: any) => i.submittedBy === userId || subordinateIds.includes(i.submittedBy)); else items = items.filter((i: any) => i.submittedBy === userId) }
            setScheduleData(prev => {
    ...
    const services = useMemo(() => {
        ...
        const roleKey = userRole.toUpperCase(); const deptKey = userDept ? userDept.toUpperCase().trim() : ""; const targetId = deptKey ? `${deptKey}_${roleKey}` : `_${roleKey}`
        let currentPerms = dynamicPermissions.find(p => p.id === targetId)
        if (!currentPerms && roleKey === "TSM") { const altTargetId = deptKey ? `${deptKey}_TERRITORY SALES MANAGER` : `_TERRITORY SALES MANAGER`; currentPerms = dynamicPermissions.find(p => p.id === altTargetId) }
        if (!currentPerms) currentPerms = dynamicPermissions.find(p => p.id === targetId) || dynamicPermissions.find(p => p.id.endsWith(`_${roleKey}`))
